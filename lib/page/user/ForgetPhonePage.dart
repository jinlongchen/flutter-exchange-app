...
class ForgetPhonePage extends StatefulWidget {

  final VoidSuccessCallback onChange;

  const ForgetPhonePage({Key key, this.onChange}) : super(key: key);
...
class _State extends State<ForgetPhonePage> with TickerProviderStateMixin {
  ...
  void getCode(){
    ...
    if(CommonUtil.isEmpty(_username.text)){
      CommonUtil.showToast("label_qsrsjh".tr());
      return;
    }

    var params = {"account":_username.text,"country":country};
    DioManager.getInstance().post(Url.USER_PWD_RESET_PHONE, params, (data){
      startCountdown();
  ...
  void register(){
    ...
    var params={"account":_username.text,"code":_code.text,"mode":"0","password":CommonUtil.generateMd5(_password.text)};

    DioManager.getInstance().post(Url.USER_PWD_RESET, params, (data){
